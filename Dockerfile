...
# runtime забирает только venv и код приложения.
# Базовый образ зафиксирован тегом патч-версии и digest.
...
# Точечный apt-upgrade util-linux проверяли: он снял бы 36 CVE-находок,
# но роняет dive-efficiency до 94.3% (20 MB перезаписей) — отвергнуто
# замером в пользу чистых слоёв; фиксы придут со следующим base-образом.
